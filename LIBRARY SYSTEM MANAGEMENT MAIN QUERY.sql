...
-- Task 14 : Update Book Status on Return:  Update the status of books in the books table to "Yes" when they are returned (based on entries in the return_status table).
...
BEGIN
    ...
    -- MySQL doesn’t support RAISE NOTICE like PostgreSQL
    -- Instead, you can return a message using SELECT:
...
-- TASK 16: Task 16: CTAS: Create a Table of Active Members. Use the CREATE TABLE AS (CTAS) statement to create a new table active_members containing members who have issued at least one book in the last 2 months.
